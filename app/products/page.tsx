...
// Product type define kar rahe hain
interface Product {
  id: string | number;
  name: string;
  price?: number;
...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    async function fetchProducts() {
      try {
        const data: Product[] = await get("/products");
        setProducts(data);
      } catch (err) {
    ...
    // Fetch AI recommendations (POST with payload)
    async function fetchRecs() {
      try {
        const recs: any = await post("/recommendations", { orders: [] });
        setRecommendations(recs.frequentlyBought || []);
      } catch (err) {
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      {recommendations.length > 0 && (
        <div className="mt-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {recommendations.map((r: Product, i: number) => (
              <motion.div
                key={i}
                data-aos="fade-up"
                whileHover={{
                ...
                className="bg-white dark:bg-gray-900 p-4 rounded-2xl shadow border border-green-200 transition-all text-center"
              >
                <p className="text-gray-800 dark:text-gray-200">{r.name}</p>
              </motion.div>
            ))}
          </div>
        </div>
